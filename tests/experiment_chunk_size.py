...
from tests.create_fixtures import docs as corpus
...
def run_experiment(chunk_size, overlap):
    ...
    for source, content in corpus.items():
        temp_path = f"/tmp/{source}"
        with open(temp_path, "w") as f:
            f.write(content)
        from pathlib import Path
        app.ingest.ingest_file(Path(temp_path), source)
        
    with app.db.SessionLocal() as db:
        chunk_count = db.query(app.db.Chunk).count()
        print(f"Ingested {len(corpus)} docs into {chunk_count} chunks.")
...
if __name__ == "__main__":
    for size in [90, 120, 180, 240, 300, 450, 600]:
        run_experiment(size, 20)
